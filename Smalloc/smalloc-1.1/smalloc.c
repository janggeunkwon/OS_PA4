...
sm_container_ptr itr ;
	int i = 0 ;
    size_t all_memory=0x0;
	size_t used_memory=0x0;
	size_t not_used_memory=0x0;

    for (itr = sm_first ; itr != 0x0 ; itr = itr->next, i++) {
		if(itr -> status == Busy)
		{
			all_memory += itr -> dsize;
